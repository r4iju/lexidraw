...
wss.on('connection', function connection(ws) {
  ws.on('message', function incoming(message) {
    ...
    try {
      ...
      // save to db
      debouncedSave(entityId, parsedMessage.payload.elements, parsedMessage.payload.appState)
      ...
      wss.clients.forEach(function each(client) {
        if (client !== ws && client.readyState === WebSocket.OPEN && clientDrawingMap.get(client) === entityId) {
          client.send(message);
        }
      });
    } catch (e) {
