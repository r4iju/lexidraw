...
export function createScreenshotToken(params: {
  userId: string;
  entityId: string;
  ttlMs?: number;
}): string {
  ...
  const expSec = nowSec + Math.floor((params.ttlMs ?? 3 * 60_000) / 1000);
...
export function verifyScreenshotToken(token: string): BasePayload | null {
  try {
    ...
    if (json.scope !== "read-document") return null;
    return json;
  } catch {
    return null;
  }
