...
export const useLexicalStyleUtils = () => {
  /**
   ...
   */
  const parseStyleString = useCallback(
    (styleString: string | null | undefined): Record<string, string> => {
   ...
   */
  const reconstructStyleString = useCallback(
  ...
  return { parseStyleString, reconstructStyleString };
};
