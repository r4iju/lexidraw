...
import { $createKeywordNode, KeywordNode } from "@packages/lexical-nodes";
...
export default function KeywordsPlugin(): React.JSX.Element | null {
  ...
  const getKeywordMatch = useCallback((text: string) => {
    ...
    const hashtagLength = (matchArr[2] as string).length;
    const startOffset = matchArr.index + (matchArr[1] as string).length;
