/** biome-ignore-all lint/correctness/useUniqueElementIds: only appear once in the file */
"use client";
...
export function AutocompleteConfig({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    setReasoningEffort(
      (cfg.reasoningEffort as typeof reasoningEffort) ?? "minimal",
    );
  ...
  return (
    <div className={cn("flex flex-col gap-3 min-w-64", className)}>
      <div className="flex items-center justify-between gap-2">
        <Label htmlFor="ac-enabled" className="font-normal cursor-pointer">
          Enable Autocomplete
        </Label>
        <Switch
          id="ac-enabled"
