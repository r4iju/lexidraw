...
export async function GET() {
  ...
  try {
    ...
    const bucketRes = await s3.send(new ListObjectsV2Command({ Bucket: env.SUPABASE_S3_BUCKET }));
    console.log(`Current number of items in ${env.SUPABASE_S3_BUCKET} bucket: ${bucketRes.Contents?.length}`);
    console.log(`Current items in ${env.SUPABASE_S3_BUCKET} bucket: ${JSON.stringify(bucketRes.Contents, null, 2)}`);
    ...
    for (const item of bucketRes.Contents ?? []) {
      ...
      const res = (await drizzle
        ...
        .where(eq(schema.uploadedImage.fileName, item.Key)))[0]
      if (!res) continue
      if (res.count === 0) {
    ...
    console.log(`${keysToDelete.length} Keys to delete: ${keysToDelete.join(', ')}`);
