...
export function ColorPickerContent({
  ...
  const hex2rgb = useCallback((hex: string): RGB => {
    const rbgArr = (
      hex
        .replace(
          /^#?([a-f\d])([a-f\d])([a-f\d])$/i,
  ...
  const rgb2hsv = useCallback(({ r, g, b }: RGB): HSV => {
    r /= 255;
    g /= 255;
    b /= 255;
  ...
  const hsv2rgb = useCallback(({ h, s, v }: HSV): RGB => {
    s /= 100;
    v /= 100;
  ...
  const rgb2hex = useCallback(({ b, g, r }: RGB): string => {
    return "#" + [r, g, b].map((x) => x.toString(16).padStart(2, "0")).join("");
  }, []);

  const toHex = useCallback((value: string): string => {
    if (!value.startsWith("#")) {
  ...
  const transformColor = useCallback(
    <M extends keyof Color, C extends Color[M]>(format: M, color: C): Color => {
    ...
    [hex2rgb, hsv2rgb, rgb2hex, rgb2hsv, toHex],
  );
