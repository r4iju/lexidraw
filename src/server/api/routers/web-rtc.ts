...
export const webRtcRouter = createTRPCRouter({
  ...
  createIceCandidate: publicProcedure
    .input(z.object({ drawingId: z.string(), candidate: z.string().min(1), userId: z.string().min(1) }))
    .mutation(async ({ input, ctx }) => {
      return await ctx.db.webRTCIceCandidate.create({
        data: {
          drawingId: input.drawingId,
  ...
  updateIceCandidate: publicProcedure
    .input(z.object({ candidateId: z.string(), drawingId: z.string(), candidate: z.string().min(1) }))
    .mutation(async ({ input, ctx }) => {
      return await ctx.db.webRTCIceCandidate.update({
  ...
  upsertIceCandidate: publicProcedure
    .input(z.object({ candidateId: z.string(), drawingId: z.string(), candidate: z.string().min(1), userId: z.string().min(1) }))
    .mutation(async ({ input, ctx }) => {
      return await ctx.db.webRTCIceCandidate.upsert({
        where: { id: input.candidateId },
        create: {
  ...
  getIceCandidates: publicProcedure
    .input(z.object({ drawingId: z.string(), userId: z.string().min(1) }))
    .query(async ({ input, ctx }) => {
      return await ctx.db.webRTCIceCandidate.findMany({
        where: {
          drawingId: input.drawingId,
          NOT: { createdBy: input.userId }
        }
  ...
  deleteIceCandidate: publicProcedure
    .input(z.object({ candidateId: z.string() }))
    .mutation(async ({ input, ctx }) => {
      await ctx.db.webRTCIceCandidate.delete({
        where: { id: input.candidateId }
      })
    }),
})
